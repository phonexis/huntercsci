...
ZooRecord::ZooRecord(std::string input_file_name){
  ...
  while(getline(fin,input)){ //for each row
    ...
    std::string classtype; //for this project: 1=Mammal,2=Bird,4=Fish

    //FOR THE ANIMAL OBJECT CONSTRUCTION
    std::string name; //stores animal name
    bool hair,domestic,airborne,aquatic,predator,venomous,fins,tail=false;
    int legs=0;    
    //NAME LINES
    ...
    if(classtype=="1"){ //MAMMAL

    }
    else if(classtype=="2"){ //BIRD

    }
    else if(classtpye=="4"){ //FISH
