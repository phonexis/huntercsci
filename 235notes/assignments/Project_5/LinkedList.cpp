...
void LinkedList<T>::invert(){
  invertRest(head_ptr_); //calls private function
}

template<class T>
void LinkedList<T>::rotate(int k){
  if(k == 0){
    return; //ends the recursive loop
  }
  else{
    //Longer method
    /*T curr_node = getNodeAt(0)->getItem();
    T next_node = getNodeAt(1)->getItem();
    for (int i = 0; i < item_count_-1; i++) {
      next_node=getNodeAt(i+1)->getItem();
      getNodeAt(i+1)->setItem(curr_node);
      curr_node=next_node;
    }
    getNodeAt(0)->setItem(curr_node);
    rotate(k-1);
    */
    //Shorter method: creates a "ring" of nodes and reassigns the headptr
    //such that you can shift the list down by 1 
    getNodeAt(item_count_-1)->setNext(getNodeAt(0)); //set the last node's next to the first node
    head_ptr_ = getNodeAt(item_count_-1); //point to the last node
    getNodeAt(item_count_-1)->setNext(nullptr); //set the new last node's next_ to null to cut off the ring
...
template<class T>
void LinkedList<T>::invertRest(Node<T>* current_first_pointer){
  if(current_first_pointer==nullptr){ //if there are no items
    return; //stops the recursive loop
  }
  if(current_first_pointer->getNext()==nullptr){ //if at the last item
    //meaning the next_ should be null
    head_ptr_=current_first_pointer; //the first node is now the last node
    return; //exits the recursive loop
  }
  invertRest(current_first_pointer->getNext()); //recursive call to move from frist to last

  //BACKTRACKING
  current_first_pointer->getNext()->setNext(current_first_pointer); //set the Next Node's next to the current
  current_first_pointer->setNext(nullptr); //deallocate the current
}
